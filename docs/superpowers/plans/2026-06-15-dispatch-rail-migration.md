...
- [x] **B. action-dispatch + remaining** (agent-converted): 4 inline → new SystemHandler (ping/read_logs/clear_logs/refresh_assets), 8 manage_* action-dispatch (Shape B lambdas), AssetManagement(8), Script(6), TestRunner(4) = 30 cmds. **Rail now 78 — ALL commands migrated; legacy switch fully dead.** compat-lint allowlist line 16→17 (AssetManagement using shifted the PrefabStage guard). 3-agent review caught 1 CRITICAL: ScriptHandler wrapped Ok(Response.Success(...)) — but Response.Success returns a serialized STRING that ResponseClassifier used to unwrap to its `data`; fixed all 12 sites to Ok(data) (faithful to the legacy unwrapped shape), verified list_scripts now returns {scripts,totalCount,message}. (TestRunner returns Ok(new{}) directly — fine.) Dogfooded ping/list_scripts/manage_asset_database/manage_tags through the rail.
- [x] **Capstone DONE**: legacy ProcessCommand switch deleted (ee1c468, dispatcher is the sole front); 10-agent sweep run (7 findings, all resolved); COMPATIBILITY.md guard lines refreshed (12c737c). Result-schema *enforcement* was REPLACED — see below.
- [x] **Result-side (replaced enforcement)**: GraphQL-style field projection via the reserved `fields` meta-param (6091f1a). Free-form, catalog-independent, applied once in CommandDispatcher (enabled by the unified rail). Core/FieldProjection.cs + dotnet tests (118). Editor-sourced field *advertisement* deferred to 0.5.0. Spec: docs/superpowers/specs/2026-06-15-field-projection-design.md.
- [x] **Docs/CHANGELOG DONE** (3f70576): CHANGELOG.md (0.3.0 + 0.4.0); README 80 tools/14 categories + field-selection note; mcp-server README ~78.

**v0.4.0 = essentially COMPLETE** (migration + field projection + docs). Deferred to ship-prep: version bump + tag + npm publish (maintainer's release actions), floor-CI matrix, release automation. Branch 133 ahead; v0.3.0 tag at a462340 (untouched).
